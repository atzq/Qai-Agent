...
```java
import net.itzq.mira.modules.ai.persistence.DefaultStreamChunkAggregator;
...
```java
import net.itzq.mira.modules.ai.agent.event.DefaultEventListener;
import net.itzq.mira.modules.ai.agent.event.type.*;
import net.itzq.mira.modules.ai.entity.chat.AnsResponse;
...
项目使用alibaba的`TransmittableThreadLocal`确保ThreadLocal变量在多线程环境中正确传递：

```java
import com.alibaba.ttl.TransmittableThreadLocal;

public class AgentGlobalContext {
    private static final ThreadLocal<AgentContextHolder> context = new TransmittableThreadLocal<>();
    
    public static void setContext(AgentContextHolder contextHolder) {
        context.set(contextHolder);
    }
    
    public static AgentContextHolder getContext() {
        return context.get();
    }
...
public class CodeAssistantAgent {
    ...
    @Tool(name = "execute_command", description = "执行命令")
